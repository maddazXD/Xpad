...
<head>
    ...
    <?= meta_tag([
        'title' => $title,
        'description' => "Nonton $title, $anime->seo_description",
        'favicon' => _storage($websettings->seo_favicon),
        'thumb' => _storage("thumbnails/$anime->thumb"),
        'keywords' => "Nonton $title,$anime->seo_keywords",
        'url' => base_url(),
        'author' => ''
    ]); ?>
            ...
            <div class="row">
                <div class="col-12 col-xl-8 col-lg-8">
                    ...
                    <div class="d-flex mb-3">
                        <?php if ($start_eps->eps == $video->eps) { ?>
                            <button class="btn btn-secondary rounded-0 w-100" disabled><i class="las la-check-double"></i></button>
                        <?php } else { ?>
                            <a href="<?= base_url($anime->seo_slug) . "/" . ($video->eps - 1) ?>" class="btn btn-secondary rounded-0 w-100"><i class="las la-angle-left"></i></a>
                        <?php } ?>
                        <a href="<?= base_url($anime->seo_slug) ?>" class="btn btn-dark border-secondary rounded-0 w-100"><i class="las la-info-circle"></i></a>
                        <?php if ($end_eps->eps == $video->eps) { ?>
                            <button class="btn btn-secondary rounded-0 w-100" disabled><i class="las la-check-double"></i></button>
                        <?php } else { ?>
                            <a href="<?= base_url($anime->seo_slug) . "/" . ($video->eps + 1) ?>" class="btn btn-secondary rounded-0 w-100"><i class="las la-angle-right"></i></a>
                        <?php } ?>
                    </div>
                    <div class="card card-body mb-1">
                        <div class="d-flex">
                            <img class="rounded-1" style="height: 150px; margin-right: 10px;" src="<?= _storage("thumbnails/$anime->thumb") ?>" alt="<?= $anime->title ?>">
                            <div>
                                <div style="font-size: 16px;" class=" title-animex"><?= $title ?></div>
                                <table class="text-sm mt-2">
                                    <tr>
                                        <td>Status </td>
                                        <td> : <?= animextype($anime->status) ?></td>
                                    </tr>
                                    <tr>
                                        <td>Jenis</td>
                                        <td> : <?= $jenis->title ?></td>
                    ...
                    <div class="card mb-1">
                        <div class="d-flex justify-content-center">
                            <ul style="padding-left: unset;">
                                <?php foreach (explode(',', $anime->genre) as $genre) : ?>
                    ...
                    <div class="card card-body mb-3">
                        ...
                        <div class="custom-scrollbar-css p-2" style="height: 300px;">
                            <?php foreach ($list_eps as $eps) : ?>
                                <div class="card p-2 mb-1 d-flex" style="background-color: #171c28;">
                                    <div class="clearfix">
                                        <a class="float-start" href="<?= base_url("$anime->seo_slug/$eps->eps") ?>"><small class="text-light">Episode <?= $eps->eps ?></small></a>
                                        <a class="float-end mx-2" href="<?= base_url("$anime->seo_slug/$eps->eps") ?>"><?= ($eps->eps == $video->eps) ? '<i class="las la-play-circle"></i>' : '<i class="las la-play"></i>' ?></a>
                                    </div>
                                </div>
                            <?php endforeach ?>
                        </div>
    ...
    <?php if ($webmaster->plugin_comment != 'off') { ?>
        <script>
            var disqus_config = function() {
                this.page.url = '<?= base_url($anime->seo_slug) ?>'; // Replace PAGE_URL with your page's canonical URL variable
                this.page.identifier = '<?= $anime->id ?>'; // Replace PAGE_IDENTIFIER with your page's unique identifier variable
            };
